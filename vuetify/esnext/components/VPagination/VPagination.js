    ...
    computed: {
        ...
        items() {
            ...
            if (this.value > left && this.value < right) {
                ...
                return [...this.range(1, left), '...', ...this.range(this.length - left + 1 + even, this.length)];
            }
        }
    },
    watch: {
    ...
    methods: {
        ...
        genItems(h) {
            return this.items.map((i, index) => {
                return h('li', { key: index }, [isNaN(i) ? h('span', { class: 'v-pagination__more' }, [i]) : this.genItem(h, i)]);
            });
        }
    },
    render(h) {
